                    ...
                    if (currHeading == True):
                        if(len(HazardArray) == 0):
                            ...
                            while(x < len(HazardArray)):
                                check = HazardArray[x]
                                if(check.lat != hazard_location.lat and check.lon != hazard_location.lon): #TODO remove old data
                                    HazardArray.pop(x)
                                    continue
                                elif(check.lat == hazard_location.lat and check.lon == hazard_location.lon):    # NEED TO CHANGE TO A RANGE OF LAT & LON VALUES
                                        identified = True
                                        if(check.fcount < hazard_str):
                                    ...
                                    print(HazardArray[x])
                                x += 1
                            if(identified == False): #did not find existing matching hazard in array. 
                                HazardArray.append(hazard_location)
                                if(check.fcount > 2): #check if newly added hazard needs to warn user
                                # WARN USER
                                    hazard_flag = True
                    receivedString = None
            time.sleep(3)
...
def identify_hazard(hazard_location):
    print("test")
